...
// updateSpecialTicketRequestStatus.php
// Used to update a special ticket request in the system.
//
// Walter | Haverfield
// 
...
// -----------------------------------------------------------------------------------------------------------------------------------------------
// 04/26/2019        Samuel Leicht       Initial creation
...
$requestNumber;
$newStatus;
$logAction = "APPROVE SPECIAL REQUEST";
...
// If SpecialRequestNumber is unset lets refuse request.
else if ( ! isset( $input->SpecialRequestNumber ) || ! isset( $input->Status ) ) { 
  ...
  $requestNumber = $input->SpecialRequestNumber;
  $newStatus = $input->Status;
  
  logAction ( $logAction, "Starting special ticket request status update (RequestNumber: $requestNumber)" );
  ...
  // Set Columns 
  $sqlStatement .= "[Status] = '$newStatus' ";

  // Set Where
  $sqlStatement .= "WHERE [SpecialRequestNumber]=" . $requestNumber  . "; ";
  ...
  if( $sqlData === false ) {
    logAction( $logAction, "Error while updating the status of special ticket request (RequestNumber: $requestNumber).");
    http_response_code( 500 );
  }
  else {
    logAction( $logAction, "Status of special ticket request (RequestNumber: $requestNumber) was successfully updated." );
